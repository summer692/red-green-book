(function () {
  ...
  const ALGO = {
    xhs: ['【小红书·自然流量要点 v' + ALGO_VER + '】',
      ...
      '- 正文：结论前置，前3行说清是什么；分点、平实、像真实的人分享；自然铺核心关键词与近义词（吃搜索流量）；结尾可自然引导评论或收藏。',
    ...
    xls: ['【微信(小绿书/公众号)·自然流量要点 v' + ALGO_VER + '】',
      ...
      '- 标签精准 4-8 个，覆盖核心词与长尾词；结尾可自然引导转发或在看。'].join('\n'),
  };
  ...
  function buildCopyPrompt() {
    ...
    return [
      ...
      series,
      '【写作要求】',
      '- 只依据下面图文里已有的信息来写，不要新增图文里没有的事实（学校、专业、时间、截止日期、费用、要求、报名方式等都不要编）。信息不够就写得稳妥些，不要硬凑；真有明显缺口可以直说缺什么。',
      '- 不要使用任何 emoji；不要用向右的箭头（→）和破折号（— 或 ——）。',
      '- 写成真实的人会说的话，平实自然，不浮夸、不喊口号、不标题党；不要出现“划重点”“一句话说清楚”“干货”“码住”“冲”“绝了”这类自夸式话术。',
      '- 正文围绕这条内容本身把事情讲清楚，需要分点就分点；不要为了凑字数注水，也不要重复标题。',
